...
data/
  ...
  synonyms.json       実務用語→条文の橋渡し（既存不適格・無窓階・面積区画など）
  sokyu.json          遡及適用（既存不適格）の判定ルール
  tenken.json         点検・報告の周期
  jinin.json          収容人員の計算式（用途ごと）
scripts/              データ生成・検証スクリプト（下記）
```

タブは 指摘→根拠 / 条文検索 / 防火管理者 / 執行手続き / 設備判定 / ノウハウ / 査察メモ / 設定 の8つ。
設備判定タブには、消火器の能力単位・主要設備の当たり・収容人員の計算・遡及適用の判定・
点検報告の期限がまとまっている。
...
| index.html の静的検査 | `python scripts/check_html.py` |
...
- **正規表現リテラルの改行**：スクリプトで `index.html` を書き換えるとき、改行を表す
  エスケープのつもりが生の改行になり、画面が真っ白になった。Pythonから書き換えるなら
  raw文字列を使うか、正規表現を避けて `split`/`join` や CSS で済ませる。
  HTMLの検査では素通りするので `check_html.py`（node があれば構文検査もする）を必ず通す。
...
実際に打ちそうなクエリで測ると原因は だいたい次の3つに分かれる。
思いつきで重みをいじる前に、まずどれなのかを切り分ける。

1. **索引に入っていない** — 別表のように、取得済みでも `DB.articles` に流していないもの。
2. **表記が違う** — 条文は「千五百平方メートル」、実務は「1500㎡」。
   `normalize()` が漢数字と単位を吸収するが、変換対象は単位や「第◯条」に限定している
   （「一部」「一般」まで数字にすると壊れるため）。
3. **条文にその語が無い** — 既存不適格・遡及・無窓階・面積区画は条文に出てこない。
   `data/synonyms.json` に同義語と、先頭に固定したい条や別表を書く。
   `refs` は `{"law_id","article"}` のほか `{"law_id","appdx":"別表第二"}` でも指せる。
